...
import { ProviderServicePlanner } from "../component/providerservicePlanner"
import { ProviderServiceRollUp } from "../component/providerservieRollUp";
import { ProviderNotificationRollUp } from "../component/providernotificationRollUp.js";
import { ProviderServiceSettings } from "../component/providerserviceSettings";

import "../../styles/privateprovider.css";

export const PrivateProvider = () => {

    const { store, actions } = useContext(Context);
    const navigate = useNavigate();
    const [selectedSection, setSelectedSection] = useState("servicePlanner");
    ...
    useEffect(() => {
        ...
        // Filter ProviderNotifications
        let filteredNotifications = store.providerNotifications.filter((notification) => {
            // Filter by status
    ...
    }, [selectedSortNotificationsBy, selectedFilterNotificationsBy, notificationsSearchBar, selectedSection, store.providerNotifications]);
            ...
            <div className="container-fluid mx-0 px-0 gx-0">
                <header>
                    ...
                    <nav className="navbar fixed-top px-3 firstBavBar">
                        ...
                        <button
                            className={`menuLabel ${selectedSection === "servicePlanner" ? "selectedSectionButton active" : ""}`}
                            onClick={() => handleSectionClick("servicePlanner")}
                        ><span>service planner</span></button>
                        <button
                            className={`menuLabel ${selectedSection === "registerServices" ? "selectedSectionButton active" : ""}`}
                            onClick={() => handleSectionClick("registerServices")}
                        ><span>register services</span></button>
                        <button
                            ...
                            <div className="row d-flex justify content-center ">
                                {filteredServices && filteredServices.map((filteredService, index) => (
                                    <div className="col-12 d-flex justify-content-center" key={filteredService.id}>
                                        <div><ProviderServiceRollUp serviceObject={filteredService} /></div>
                                    </div>
                                ))}
                            </div>
                        </div>)
